...
void SPITransmit(SPIInstance *spi_ins, uint8_t *ptr_data, uint8_t len)
{
    ...
    case SPI_DMA_MODE:
        // DMA传输前Clean Cache
        Cache_CleanByAddr(ptr_data, len);
...
void SPIRecv(SPIInstance *spi_ins, uint8_t *ptr_data, uint8_t len)
{
    ...
    {
    case SPI_DMA_MODE:
        // DMA接收前Invalidate Cache,接收后也需要Invalidate
        Cache_InvalidateByAddr(ptr_data, len);
...
void SPITransRecv(SPIInstance *spi_ins, uint8_t *ptr_data_rx, uint8_t *ptr_data_tx, uint8_t len)
{
    ...
    *spi_ins->cs_pin_state =
    ...
    {
    case SPI_DMA_MODE:
        // DMA传输前Clean发送缓冲区,Invalidate接收缓冲区
        Cache_CleanByAddr(ptr_data_tx, len);
        Cache_InvalidateByAddr(ptr_data_rx, len);
