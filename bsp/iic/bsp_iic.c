...
void IICTransmit(IICInstance *iic, uint8_t *data, uint16_t size, IIC_Seq_Mode_e seq_mode)
{
    ...
    case IIC_IT_MODE:
        ...
        break;
    case IIC_DMA_MODE:
        // I2C DMA 发送前必须 Clean Cache
        Cache_CleanByAddr(data, size);
        if (seq_mode == IIC_SEQ_RELEASE)
...
void IICReceive(IICInstance *iic, uint8_t *data, uint16_t size, IIC_Seq_Mode_e seq_mode)
{
    ...
    case IIC_IT_MODE:
        ...
        break;
    case IIC_DMA_MODE:
        // I2C DMA 接收前 Invalidate Cache
        Cache_InvalidateByAddr(data, size);
        if (seq_mode == IIC_SEQ_RELEASE)
...
void HAL_I2C_MasterRxCpltCallback(I2C_HandleTypeDef *hi2c)
{
    ...
    for (uint8_t i = 0; i < idx; i++)
    {
        ...
        {
            // DMA 接收完成后 Invalidate Cache，确保读取最新数据
            if (iic_instance[i]->work_mode == IIC_DMA_MODE)
            {
                Cache_InvalidateByAddr(iic_instance[i]->rx_buffer, iic_instance[i]->rx_len);
            }
