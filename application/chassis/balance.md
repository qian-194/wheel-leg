...
本文档记录当前平衡轮腿底盘 application 层的实现状态。旧的麦轮 chassis 流程已经不再作为本文档主体，当前重点是 `balance.c`、`balance_state.c`、`balance_control.c` 和 `wbr_lqr_calc.h`。
...
注意：`BalanceControlUpdate()` 当前只更新 `BalanceState` 中的力/力矩字段，不直接写电机输出。
...
- 当前编译仍提示 `CalcWbrLQR` 未使用，说明 LQR 输出还没有接入 `BalanceTask()` 的主控制链路。
